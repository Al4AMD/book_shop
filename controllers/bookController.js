...
exports.getUserBook = async (req, res) => {
  try {
    ...
    if (books) {
      return responseHandler(res, 200, "User Books Fetched!", books);
    } else {
      return responseHandler(res, 404, "User has no Books!");
    }
  } catch (error) {
    return responseHandler(res, 500, "Error fetching user Books!", error);
  }
};
